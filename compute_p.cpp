//This version fits the parallel tempering data, with more columns
...
int main(int argc, char * argv[]){
	...
	for(int i = 0; i < npo; i++)  {
		...
		while(fscanf(input, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",
					 &dt[0], &dt[1], &dt[2], &dt[3], &dt[4], &dt[5], &dt[6], &dt[7], &dt[8], &dt[9], &dt[10], &dt[11], &dt[12], &dt[13], &dt[14], &dt[15], &dt[16]) != EOF) {
	...
	for(int i = 0; i < Nrow; i++) {
		...
		fprintf(output, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",
				...
				DataArray[6][i]/dn, DataArray[7][i]/dn,  DataArray[8][i]/dn,  DataArray[9][i]/dn,  DataArray[10][i]/dn,  DataArray[11][i]/dn, DataArray[12][i]/dn, DataArray[13][i]/dn, DataArray[14][i]/dn, DataArray[15][i]/dn, DataArray[16][i]/dn);
	}
	fclose(output);
}
